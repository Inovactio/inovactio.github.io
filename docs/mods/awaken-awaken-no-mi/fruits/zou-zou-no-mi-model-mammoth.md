# Zou Zou no Mi, Model: Mammoth

**Ancient Zoan** · 5 awakened abilities

These abilities unlock once the fruit is **awakened**: see [Awakening](../index.md#getting-started).

!!! note "About the values"
    The values are those of the ability's tooltip in game, with the base mod's default settings. Damage is shown on the base mod's scale, as in the ability screen.

## Awaken Mammoth Heavy Point { #awaken-mammoth-heavy-point }

*Active · Transformation*

Transforms the user into an awakened mammoth hybrid, which focuses on strength.

| Stat | Value |
|---|---|
| Cooldown | 1 s |
| Hold | ∞ s |

**Stats while active**

| Stat | Value |
|---|---|
| Attack Speed | x1.1 |
| Step Height | +1 |
| Armor | +25 |
| Max Health | x1.25 |
| Block Reach | +2.5 |
| Knockback Resistance | +3 |
| Toughness | +8 |
| Jump Height | +2 |
| Punch Damage | +18 |
| Entity Reach | +2.5 |
| Speed | +0.28 |
| Fall Resistance | +10 |

## Awaken Mammoth Guard Point { #awaken-mammoth-guard-point }

*Active · Transformation*

Transforms the user into an awakened mammoth, which focuses on defense.

| Stat | Value |
|---|---|
| Cooldown | 1 s |
| Hold | ∞ s |

**Stats while active**

| Stat | Value |
|---|---|
| Attack Speed | +0.1 |
| Step Height | +1 |
| Armor | +40 |
| Max Health | x1.4 |
| Block Reach | +2.7 |
| Knockback Resistance | +3 |
| Toughness | +10 |
| Jump Height | +1.5 |
| Punch Damage | +15 |
| Entity Reach | +2.7 |
| Speed | +0.14 |
| Fall Resistance | +20 |

## Drought Calamity { #drought-calamity }

*Active*

Stomps the ground so hard all the water in the area dries up, leaving a wasteland and rotting enemies.

| Stat | Value |
|---|---|
| Cooldown | 60 s |
| Charge | 3 s |
| Range | 24 blocks (area) |
| Damage | 70 |

## Drought Siphon { #drought-siphon }

*Active*

The mammoth raises its trunk and draws the water out of every enemy around it, healing by what they lose.

Enemies soaked by water or rain have twice as much to give, and are left parched and slowed.

| Stat | Value |
|---|---|
| Drain per Pulse | 12 |
| Full Channel Drain per Enemy | 96 |
| Drain on Wet Enemies | x2 |
| Range | 18 blocks (area) |
| Hold | 8 s |
| Cooldown | 45 s |

## Ancient Hide { #ancient-hide }

*Passive*

In an awakened form, the mammoth's ancient hide never lets a single blow take more than a small part of its health.

Its wool keeps out the cold: it cannot be frozen or frostbitten.

| Stat | Value |
|---|---|
| Largest Blow Taken (% of max health) | 15 |
