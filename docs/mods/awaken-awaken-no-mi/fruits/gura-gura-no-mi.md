# Gura Gura no Mi

**Paramecia** · 3 awakened abilities

These abilities unlock once the fruit is **awakened**: see [Awakening](../index.md#getting-started).

!!! note "About the values"
    The values are those of the ability's tooltip in game, with the base mod's default settings. Damage is shown on the base mod's scale, as in the ability screen.

## Tenchi Hokai { #tenchi-hokai }

*Active*

Tears the ground apart over a huge area, throwing it into the air and knocking out enemies.

| Stat | Value |
|---|---|
| Cooldown | 240 s |
| Charge | 12 s |
| Range | 80 blocks (area) |
| Damage | 24–80 |

## Funshin { #funshin }

*Active*

The user releases a quake under their feet, launching themselves in the direction they look.

Landing sends a second quake through the enemies around the user.

| Stat | Value |
|---|---|
| Cooldown | 25 s |
| Charge | 1 s |
| Damage | 40 |
| Explosion power | 20 |
| Explosion size | 10 |
| Explosion damage | 60 |

## Aftershock { #aftershock }

*Passive*

The user's quakes do not stop when the blow does: every enemy hit by a Gura ability is shaken again a moment later.

The aftershock passes through Logia bodies and leaves the target dizzy.

| Stat | Value |
|---|---|
| Aftershock Damage | 25 |
| Aftershock Delay (s) | 1.5 |
