# Hito Hito no Mi, Model: Daibutsu

**Mythical Zoan** · 3 awakened abilities

These abilities unlock once the fruit is **awakened**: see [Awakening](../index.md#getting-started).

!!! note "About the values"
    The values are those of the ability's tooltip in game, with the base mod's default settings. Damage is shown on the base mod's scale, as in the ability screen.

## Awaken Daibutsu Point { #awaken-daibutsu-point }

![](../abilities/awaken-daibutsu-point.png){ .ability-icon } *Active · Transformation*

Transforms the user into an awakened golden Buddha, which focuses on raw power.

| Stat | Value |
|---|---|
| Cooldown | 1 s |
| Hold | ∞ s |

**Stats while active**

| Stat | Value |
|---|---|
| Attack Speed | <span class="stat-malus">-0.8</span> |
| Armor | +20 |
| Block Reach | +14 |
| Armor Toughness | +12 |
| Toughness | +8 |
| Jump Height | +7 |
| Punch Damage | +25 |
| Entity Reach | +14 |
| Fall Resistance | +35 |
| Step Height | +2 |
| Max Health | x1.4 |
| Knockback Resistance | +7 |
| Speed | +0.7 |

## Nyorai Shinshō { #nyorai-shinsho }

![](../abilities/nyorai-shinsho.png){ .ability-icon } *Active*

Applies ![](../effect-icons/buddha-seal.png){ .effect-mini .pixelated }[Buddha Seal](../effects.md#effect-buddha-seal)

Drops a giant Buddha palm from the sky onto the aimed spot, crushing everything under it.

Anything it lands on is pinned to the ground for 40 seconds, unable to walk, swim or jump

| Stat | Value |
|---|---|
| Cooldown | 90 s |
| Charge | 1 s |
| Damage | 120 |
| Range | 14 blocks (area) |

## Sanctuary { #sanctuary }

![](../abilities/sanctuary.png){ .ability-icon } *Active*

The Buddha stands over its own: every blow aimed at an ally nearby lands on the user instead.

The blow keeps its nature, so the user's armour is what decides the price. The vow breaks once they have taken enough.

| Stat | Value |
|---|---|
| Cooldown | 90 s |
| Hold | 60 s |
| Shelters Allies Within (blocks) | 24 |
| Breaks After Taking (HP) | 300 |
