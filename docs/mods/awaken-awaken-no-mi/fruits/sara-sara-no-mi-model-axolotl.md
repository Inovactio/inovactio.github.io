# Sara Sara no Mi, Model: Axolotl

**Zoan** · 5 awakened abilities

These abilities unlock once the fruit is **awakened**: see [Awakening](../index.md#getting-started).

!!! note "About the values"
    The values are those of the ability's tooltip in game, with the base mod's default settings. Damage is shown on the base mod's scale, as in the ability screen.

## Awaken Axolotl Heavy Point { #awaken-axolotl-heavy-point }

*Active · Transformation*

Transforms the user into an awakened axolotl hybrid, which focuses on strength and regeneration

| Stat | Value |
|---|---|
| Cooldown | 1 s |
| Hold | ∞ s |

**Stats while active**

| Stat | Value |
|---|---|
| Step Height | +2 |
| Armor | +30 |
| Block Reach | +0.6 |
| Regen Rate | +0.5 |
| Knockback Resistance | +1.5 |
| Toughness | +5 |
| Jump Height | +3 |
| Punch Damage | +10 |
| Entity Reach | +0.6 |
| Speed | +1.26 |
| Fall Resistance | +4 |

## Awaken Axolotl Walk Point { #awaken-axolotl-walk-point }

*Active · Transformation*

Transforms the user into an awakened axolotl, which focuses on speed and swimming.

| Stat | Value |
|---|---|
| Cooldown | 1 s |
| Hold | ∞ s |

**Stats while active**

| Stat | Value |
|---|---|
| Attack Speed | +0.1 |
| Step Height | +1 |
| Regen Rate | +1 |
| Jump Height | +2.4 |
| Speed | +1.26 |
| Fall Resistance | +4.5 |

## Cellular Overload { #cellular-overload }

*Active*

Pushes the axolotl's cells past every limit: every harmful effect that can be shed is shed at once, and for a short time most of the health the user loses grows back within seconds.

Once the overload ends the body is starving.

| Stat | Value |
|---|---|
| Cooldown | 60 s |
| Hold | 10 s |
| Damage Regrown (%) | 75 |
| Regrown Over (s) | 3 |
| Natural Regeneration (x) | 3 |

## Toxic Bloom { #toxic-bloom }

*Active*

Releases all the poison stored in the axolotl's Toxic Reserve at once, as a toxic bloom that hurts and poisons the enemies around.

The more health was stored, the harder it hits.

| Stat | Value |
|---|---|
| Cooldown | 15–45 s |
| Damage | 15–150 |
| Range | 12 blocks (area) |
| Healing Released As Damage (%) | 150 |

## Toxic Reserve { #toxic-reserve }

*Passive*

While in an awakened form, every bit of health the axolotl regenerates is stored as poison in its body, ready to be released by Toxic Bloom.

The poison slowly seeps away once the fighting stops.

| Stat | Value |
|---|---|
| Healing Stored (max) | 100 |
| Seeps Away After No Fighting For (s) | 10 |
