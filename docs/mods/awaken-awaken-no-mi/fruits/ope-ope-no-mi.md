# Ope Ope no Mi

**Paramecia** · 4 awakened abilities

These abilities unlock once the fruit is **awakened**: see [Awakening](../index.md#getting-started).

!!! note "About the values"
    The values are those of the ability's tooltip in game, with the base mod's default settings. Damage is shown on the base mod's scale, as in the ability screen.

## K-Room { #k-room }

![](../abilities/k-room.png){ .ability-icon } *Active*

**Modes**

- **Arm Mode**: Coats the arm in a tiny Room. Awakened, the coating is what Anesthesia rides on.
- **Weapon Mode**: Coats the held weapon in a tiny Room. Awakened, the coating is what Anesthesia rides on.

| Stat | Value |
|---|---|
| Charge | 1 s |
| Hold | 60 s |
| Cooldown | 20 s |

## Shock Wille { #shock-wille }

![](../abilities/shock-wille.png){ .ability-icon } *Active*

Sends an electric shock through the K-Room so the next hit deals massive damage through armor

| Stat | Value |
|---|---|
| Damage | 110 |
| Hold | 10 s |
| Cooldown | 60 s |

## Puncture Wille { #puncture-wille }

![](../abilities/puncture-wille.png){ .ability-icon } *Active*

The user plants the coated hand and a colossal spire of Room erupts where they are aiming.

Everything around it is run through, ignoring armor, and thrown into the air.

| Stat | Value |
|---|---|
| Spire Damage | 130 |
| Spire Radius (blocks) | 6 |
| Opening Range (blocks) | 40 |
| Charge | 2 s |
| Cooldown | 60 s |

## Anesthesia { #anesthesia }

![](../abilities/anesthesia.png){ .ability-icon } *Passive*

While K-Room is active, every blow the user lands cuts with the Room and puts its target under.

A numbed body no longer flinches: the user and their allies ignore the invulnerability frames of anyone they have anaesthetised, so every hit lands in full.

| Stat | Value |
|---|---|
| Coated Fist Damage | 30 |
| Coated Weapon Damage | 20 |
| Anesthesia Duration (s) | 6 |
| Numbed Attack Speed (%) | -40 |
| Numbed Attack Damage (%) | -25 |
| Numbed Movement Speed (%) | -25 |
